...
class IjJsonExtensionProvider : JsonExtensionProvider {
  ...
  override fun init() {
    ...
    val extensions: List<ExtensionBean> = resourceStream.use {
      val bundledExtensions: List<KspExtensionBean> = Json.decodeFromStream(it)
      bundledExtensions.map { kspBean -> kspBean.toExtensionBean(IDEAVIM_ID) }
    }
