    ...
    try {
      ...
      if (inputStream != null) {
        return inputStream.use { stream ->
          val extensionBeans: List<KspExtensionBean> = Json.decodeFromStream(stream)
          logger.debug(
            "Plugin $pluginId: Extensions: ${extensionBeans.joinToString(", ") { it.extensionName }}."
          )
          extensionBeans.map { it.toExtensionBean(pluginId) }
        }
      } else {
        logger.debug("Plugin $pluginId: META-INF/extensions.json not found in the plugin JAR.")
      }
    } catch (e: Exception) {
      logger.error("Error scanning plugin JAR", e)
    }
    return null
  }
